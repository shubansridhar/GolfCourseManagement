...
function showMemberView() {
    ...
    const mV = document.getElementById('member-view'); if (mV) mV.style.display = 'block';
    updateHeader();
    member.loadMemberData();
    // Load member metrics
    loadStatisticsData('member');
}
function showEmployeeView() {
    if (!currentUser || !isEmployee()) { showAuthView(); return; }
    hideAllViews();
    const aV = document.getElementById('app-view'); if (aV) aV.style.display = 'block';
    const eV = document.getElementById('employee-view'); if (eV) eV.style.display = 'block';
    updateHeader();
    loadEmployeeData();
    // Load employee metrics
    loadStatisticsData('employee');
}
...
function updateHeader() {
    ...
    if (uD && uS) {
        uD.textContent = `${currentUser.username} (${currentUser.role})`;
        uS.style.display = 'flex';
    }
    // Show statistics button for all roles (admin, employee, member)
    const sB = document.getElementById('statistics-btn');
    if (sB) {
        sB.style.display = currentUser ? 'inline-flex' : 'none';
    }
}
